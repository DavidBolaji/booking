...
exports.getAllExceptOneBookings = async (req, res, next) => {
	...
	if(new Date(from).getMonth() < 10) {
		...
		valFrom = `${from.split('-')[0]}-${parseInt(from.split('-')[1])}-${from.split('-')[2]}`;
	}

	if(new Date(from).getMonth() < 10) {
		valTo = to;
	} else {
		valTo = `${to.split('-')[0]}-${parseInt(to.split('-')[1])}-${to.split('-')[2]}`;
	}
	...
	const halls = await Booking.find({_id: {$ne: baseUrl}, hallname, bookedFrom: {$gte: new Date(prevMonth)}, bookedTo: {$lte: new Date(nextMonth)}})
...
exports.updateBooking = async (req, res, next) => {
	...
	try {
		const { id } = req.params;
		const updatedBooking = await Booking.updateOne({_id: id}, 
			{
				$set: {
...
exports.getAllDates = async (req, res, next) => {
	...
	const halls = await Booking.find({bookedFrom: {$gte: new Date(prevMonth)}, bookedTo: {$lte: new Date(nextMonth)}})
	responseHandler(res, 200, halls);
}
